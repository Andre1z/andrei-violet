...
def center_window():
    ...
    # Para otros sistemas, puedes probar a configurar la variable de entorno antes de inicializar pygame
...
class Bot:
    def __init__(self, x, y, angle, sensor_angles=None, sensor_range=None, bot_radius=None, sensor_weights=None, bias=None):
        ...
        # Cada robot tiene su propia configuración de sensores.
        ...
        # Cada robot tiene pesos para cada sensor y un sesgo.
    ...
    def update(self, maze):
        ...
        if not collision:
            ...
            # En caso de colisión, ajustar el ángulo de forma aleatoria.
            self.angle += random.uniform(-0.5, 0.5)

    def get_sensor_reading(self, maze, sensor_angle):
        """
        Lanza un rayo en la dirección del sensor y devuelve un valor normalizado:
        0 significa que no se detecta pared en el rango; valores cercanos a 1 indican una pared cercana.
        """
    ...
    def draw(self, screen):
        # Dibujar el robot como un círculo azul utilizando su propio bot_radius.
...
def main():
    ...
    # Configura la variable de entorno para centrar la ventana al iniciar (si aplica en tu plataforma)
    os.environ['SDL_VIDEO_CENTERED'] = '1'
    screen = pygame.display.set_mode((SCREEN_WIDTH, SCREEN_HEIGHT))
    center_window()  # Asegurar que la ventana esté centrada al inicio
    ...
    start_x = CELL_SIZE // 2
    start_y = CELL_SIZE // 2
    bots = [Bot(start_x, start_y, 0) for _ in range(50)]
    winner = None

    running = True
    while running:
        # Actualizar el título de la ventana con la generación actual
        ...
        # Cuando un robot gana, utiliza sus parámetros para generar la siguiente generación con leves mutaciones.
        if winner is not None:
            print(f"¡Ganador de la generación {generation} encontrado!")
            new_bots = []
            for _ in range(120):
                # Mutar ligeramente cada parámetro.
            ...
            # Cada 2 generaciones, aumentar el tamaño del laberinto (pantalla) y centrar la ventana.
            if generation % 1 == 0:
                SCREEN_WIDTH += 4
                SCREEN_HEIGHT += 4
                screen = pygame.display.set_mode((SCREEN_WIDTH, SCREEN_HEIGHT))
                center_window()  # Centrar la ventana tras cambiar su tamaño
                maze = Maze(SCREEN_WIDTH, SCREEN_HEIGHT, CELL_SIZE, WALL_THICKNESS)
                # Reiniciar la posición de los robots en el centro de la celda superior izquierda del nuevo laberinto.
                start_x = CELL_SIZE // 2
                start_y = CELL_SIZE // 2
                for bot in bots:
