...
class Bot:
    ...
    def update(self, maze, foods):
        ...
        sensor_readings = []
        # Obtener lecturas de sensores con detección de paredes, comida y meta.
    ...
    def get_sensor_reading(self, maze, sensor_angle, foods):
        """
        Lanza un rayo en la dirección del sensor. Ahora se comprueba en cada paso:
          1. Si se choca con una pared, se retorna la lectura basada en la distancia (sensores no pueden ver detrás de paredes).
          2. Si se alcanza la salida o comida antes de chocar con una pared, se retorna un valor alto.
        """
        for distance in range(0, int(self.sensor_range), 5):
            ...
            # Primero se comprueba si hay pared (bloqueo inmediato)
            if any(test_rect.colliderect(wall) for wall in maze.walls):
                return (self.sensor_range - distance) / self.sensor_range
            # Luego se comprueba si se alcanza la salida
            if maze.exit.collidepoint(test_x, test_y):
                return 2.0
            # Y se verifica si se detecta comida
            for food in foods:
                if math.hypot(test_x - food[0], test_y - food[1]) <= FOOD_RADIUS:
                    return 1.5
        # Si no se detecta nada dentro del rango, retornar 0 (o se puede agregar bonus por exploración)
        return 0.0

    def sensor_endpoint(self, maze, foods, sensor_angle):
        """
        Calcula el punto final del rayo del sensor, avanzando en pasos y cortando el rayo al encontrar una pared,
        salida o comida.
        """
        for distance in range(0, int(self.sensor_range), 5):
            test_x = self.pos[0] + distance * math.cos(sensor_angle)
            ...
            for food in foods:
                if math.hypot(test_x - food[0], test_y - food[1]) <= FOOD_RADIUS:
                    return test_x, test_y
        # Si no se encuentra nada, el rayo llega hasta el máximo rango
    ...
    def draw(self, screen, maze, foods):
        ...
        # Dibujar los rayos de los sensores en verde, truncándolos al encontrar una pared, salida o comida.
        for offset in self.sensor_angles:
            sensor_angle = self.angle + offset
