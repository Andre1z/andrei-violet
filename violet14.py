...
class Bot:
    ...
    def update(self, maze, foods):
        ...
        sensor_readings = []
        # Obtener lecturas de sensores con la nueva lógica de detección
    ...
    def get_sensor_reading(self, maze, sensor_angle, foods):
        ...
        En cada paso se comprueba:
          1. Si el punto está dentro de la salida (meta), retorna 2.0.
          2. Si el punto detecta comida, retorna 1.5.
          3. Si el punto choca con una pared, retorna una lectura proporcional a la distancia.
        ...
        for distance in range(0, int(self.sensor_range), 5):
            ...
            # Primero, si el rayo entra en la zona de la salida, retorna 2.0
            if maze.exit.collidepoint(test_x, test_y):
                return 2.0
            # Luego, comprueba si se detecta comida
            for food in foods:
                if math.hypot(test_x - food[0], test_y - food[1]) <= FOOD_RADIUS:
                    return 1.5
            # Finalmente, si se choca con una pared, retorna una lectura basada en la distancia
    ...
    def sensor_endpoint(self, maze, foods, sensor_angle):
        """
        Calcula el punto final del rayo del sensor avanzando en pasos y cortando el rayo al encontrar
        una pared, salida o comida.
        """
        for distance in range(0, int(self.sensor_range), 5):
            test_x = self.pos[0] + distance * math.cos(sensor_angle)
            test_y = self.pos[1] + distance * math.sin(sensor_angle)
    ...
    def draw(self, screen, maze, foods):
        ...
        # Dibujar los rayos de los sensores en verde, truncándolos al encontrar un obstáculo o meta.
        for offset in self.sensor_angles:
            sensor_angle = self.angle + offset
